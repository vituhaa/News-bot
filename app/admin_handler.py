...
async def process_super_choice(message: Message, state: FSMContext):
    if message.text == "Добавить админа":
        ...
        await message.answer(
            "Неверная команда. Выберите любую из предложенных в клавиатуре.",
...
async def process_new_admin(message: Message, state: FSMContext):
    try:
        ...
        await message.answer(
            "Неверный ввод! Введите числовой ID (например, 123456789):"
        )
        return
...
async def revision_post(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        f"Напишите комментарий для автора (что нужно исправить):"
    )
    await callback.answer()
...
async def process_comment(message: Message, state: FSMContext):
    ...
    if len(comment) < 10:
        await message.answer(
            "Комментарий должен содержать минимум 10 символов. Напишите подробнее:"
        )
        return
    ...
    await message.answer(
        f"Новость #{post_id} возвращена автору.\n"
